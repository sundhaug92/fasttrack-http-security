...
def calculate_sri_hashes(flow: http.HTTPFlow) -> None:
    ...
    old_hashes=db_cursor.execute('SELECT sha256,sha384,sha512 FROM sri_hashes WHERE resource_uri=?',(flow.request.pretty_url,)).fetchone()
    current_hashes=[calculate_sri(flow.response.content, _) for _ in ['sha256','sha384','sha512']]
    if (old_hashes is not None) and any([old_hashes[_]!=current_hashes[_] for _ in range(len(old_hashes))]):
        print('>>>> {}: hash mismatch, {}, {}'.format(flow.request.pretty_url,old_hashes,current_hashes))
        db_cursor.execute('UPDATE sri_hashes SET possibly_dynamic=1 WHERE resource_uri=?',(flow.request.pretty_url,))
    else:
        db_cursor.execute('INSERT INTO sri_hashes VALUES (?,?,?,?,?,0)',('/'.join(flow.request.pretty_url.split('/')[2:3]),flow.request.pretty_url,*current_hashes))
    db_conn.commit()

def insert_sri_hashes(flow: http.HTTPFlow, hash_name) -> None:
     if hash_name not in ['sha256','sha384','sha512']:raise Exception('Invalid hash "{}"'.format(hash_name))
     for resource_url,sri_hash in db_cursor.execute('SELECT resource_uri,{} FROM sri_hashes WHERE possibly_dynamic=0'.format(hash_name)).fetchall():
         flow.response.text=flow.response.text.replace('"{}"'.format(resource_url), '"{}" integrity="{}" crossorigin="anonymous"'.format(resource_url,sri_hash))
...
#        flow.response.headers['Content-Security-Policy-Report-Only']="default-src 'none'; style-src 'none'; img-src 'none'; object-src 'none'; script-src 'none'; frame-src 'none'; frame-ancestors 'none'; media-src 'none'; manifest-src 'none'; connect-src 'none'; worker-src 'none'; font-src 'none'; form-action 'none'; report-uri https://example.org{}".format(csp_report_path)
        if REPORT_ONLY:
            ...
            flow.response.headers['Content-Security-Policy-Report-Only']=get_csp_policy(flow).replace(csp_blockd_path,csp_report_path)
        else:
...
db_cursor.execute('CREATE TABLE IF NOT EXISTS sri_hashes (resource_domain TEXT, resource_uri, sha256, sha384, sha512, possibly_dynamic)')
db_conn.commit()
